...
Last out is the ``friends'' test case. As there's a lot of data points for this case, the results have been split in two; one logarithmic view giving a rough overview of how the nodes performed (\autoref{fig:friends-log}), and one cropped view, where only edges with latency less than 500ms is included (\autoref{fig:friends-lin}).
...
\begin{figure}
    ...
    \end{axis}
    \end{tikzpicture}
    \caption{Outgoing latencies for the ``friends'' test case, results below 500ms only, linear scale}
