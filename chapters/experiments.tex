...
\todo{Measurements were done in two different ways, as the experiment was first intended to be platform-agnostic, to allow comparison between traditional MCU-based topologies and peer-to-peer topologies, but as it turned out to be infeasible to test the MCU-based solutions, a pure WebRTC solution would have been optimal. SKRIV FERDIG! Inkluder hvordan js ble injisert for å dumpe getStats til ekstern server.}
...
However sobering the Firefox results are, it's unlikely they provide the full story of WebRTC. After all, WebRTC is relatively widely deployed for such a young technology and in use by lots of different providers, if this was the expected results it would likely not have gained much traction. Thus, to have a point of comparison, we re-ran the tests using Chrome, but instead of the very manual and labor-intensive method of filming timestamps, used the \texttt{getStats} API to extract the relevant data. The browser extracts these data from the \gls{rtcp}-stream present between each pair of nodes, which contains control signals for the \gls{rtp} stream.
...
\subsubsection{Audio Receive}
\begin{itemize}
    \item bytesReceived (\texttt{``16983''})
    \item googCodecName (\texttt{``opus''})
    \item googCurrentDelayMs (\texttt{``100''})
    \item googJitterBufferMs (\texttt{``43''})
    \item googJitterReceived (\texttt{``0''})
    \item googPreferredJitterBufferMs (\texttt{``20''})
    \item packetsLost (\texttt{``0''})
    \item packetsReceived (\texttt{``179''})
\end{itemize}
...
\begin{figure}
    ...
    \begin{subfigure}[t]{\textwidth}
        ...
        \begin{axis}[
            ...
            \input{data/appear.in-capture-standup/latency-getstats.tex}
        \end{axis}
        \end{tikzpicture}
        \subcaption{Latencies measured without traffic shaping, seven nodes}
    \end{subfigure}
    ...
    \end{subfigure}
    \caption{Observed latencies in a conversation with seven nodes, with and without traffic shaping FINAL VS CAPTURE}
    \label{fig:vanilla-7p-getstats}
\end{figure}
