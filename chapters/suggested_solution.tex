...
\begin{figure}
    ...
    \begin{tikzpicture}
        ...
        \addplot+[domain=0:51, mark=none]{6*ln(x)};
        \end{axis}
    \end{tikzpicture}
...
To illustrate how we model this, imagine that instead of node splitting like we did above, we're doing \emph{edge splitting}, replacing a single edge connecting to two nodes with a set of edges, which can have different capacities and costs. This results in a linear approximation like we saw in MISSING\_FIG\_FROM\_ABOVE\todo{Insert link to actual fig when added}.
...
Transcoders are more costly to run than repeaters, due to much heavier compute operations required to transcode video in real-time. They will typically be nodes with specialized hardware for video encoding, such as Intel processors with Quick Sync\footnote{Intel's hardware-accelerated H.264 encoder, first introduced in the Sandy Bridge family of CPUs}, GPUs, or other hardware built for real-time streaming.
...
\todo[inline]{Write about how the implementation was done}


\section{Results}\label{sec:results}

\todo[inline]{Provide the results from running the script on the test cases}


\section{Tuning}

\todo[inline]{How can the algorithm be tuned, and how was the parameters chosen? (don't say random, don't say random, don't say random...)}
