...
Latencies are depicted in \autoref{fig:standup-latency}. While Chrome generally performs okay, we see that even though nodes C and D are not saturating their connections, they observe latencies which are significantly more delayed compared to the two other nodes. Both C and D would have a noticable delay in this test case, which detracts from the QoE. On Firefox only nodes A to C can communicate.
