...
ip=129.241.209.28 # The IP address bound to the interface
...
function start_tc { 
    ...
    $TC class add dev $interface parent 1: classid 1:1 htb rate $interface_speed burst 150k

    $TC class add dev $interface parent 1:1 classid 1:10 htb rate $download_limit burst 150k
    $TC class add dev $interface parent 1:1 classid 1:20 htb rate $upload_limit burst 150k
